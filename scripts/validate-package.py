...
def validate_skills() -> None:
    ...
    if dirs != EXPECTED:
        fail(
            f"top-level skills differ: expected {sorted(EXPECTED)}, got {sorted(dirs)}"
        )
    names = []
...
def validate_json_toml() -> None:
    ...
    if (
        not all(k in entry["policy"] for k in ("installation", "authentication"))
    ...
    for path in (ROOT / ".codex/agents").glob("*.toml"):
        ...
        if (
            "Allowed MCP:" not in data["developer_instructions"]
            and "MCP" in data["developer_instructions"]
        ):
...
def validate_dependency_locks() -> None:
    lock = ROOT / "sdk/python/requirements.lock"
    checksum = ROOT / "sdk/python/requirements.lock.sha256"
    if not lock.is_file() or not checksum.is_file():
        fail("Python dependency lock or checksum is missing")
    fields = checksum.read_text(encoding="utf-8").strip().split()
    if len(fields) != 2 or fields[1] != "requirements.lock":
        fail("Python dependency-lock checksum format is invalid")
    actual = hashlib.sha256(lock.read_bytes()).hexdigest()
    if fields[0].lower() != actual:
        fail("Python dependency lock checksum does not match")
    if not any(
        re.fullmatch(r"[A-Za-z0-9_.-]+==[^;\s]+(?:\s*;.*)?", line)
        for line in lock.read_text(encoding="utf-8").splitlines()
    ):
        fail("Python dependency lock contains no exact pins")
...
def validate_site() -> None:
    ...
    for route in routes:
        path = (
            ROOT / "site" / route / "index.html" if route else ROOT / "site/index.html"
        )
        text = path.read_text(encoding="utf-8")
        if (
            'lang="en-ZA"' not in text
            or "<title>" not in text
            or 'rel="canonical"' not in text
        ):
    ...
    if (
        social.count('class="social-card"') != 7
        or 'rel="me noopener noreferrer"' not in social
    ):
...
def validate_security() -> None:
    ...
    for path in ROOT.rglob("*"):
        if (
            ...
            or path.stat().st_size > 3_000_000
        ):
            continue
        ...
        if path.name not in {
            ...
            for pattern in forbidden_paths:
                if pattern.search(text):
                    fail(
                        f"developer-specific absolute path in {path.relative_to(ROOT)}"
                    )
