...
def test_delivery_communications_lifecycle_rag_citations_and_suggestions(tmp_path):
    ...
    gateway = CommunicationGateway(
        database, adapters, {"offer": "{product}: {currency} {price}"},
    )
    ...
    intent = MessageIntent(
        message_id="message-1", tenant_id="tenant-a", purpose="price_quote",
        customer_id="customer-a", locale="en-ZA", channels=["email", "whatsapp"],
        template_id="offer", variables={"product": "Radio", "currency": "ZAR", "price": "156.25"},
        consent_reference="consent-1", approval_required=True,
    )
    references = gateway.send(
        intent, authority={"send_customer_message", "approve:message-1"}, now=now,
    )
    ...
    for index, reference in enumerate(references.values(), start=1):
        assert gateway.reconcile_webhook(
            provider="mock", event_id=f"evt-{index}", provider_reference=reference,
            status=MessageStatus.DELIVERED, payload={"status": "delivered"}, received_at=now,
        )
    assert not gateway.reconcile_webhook(
        provider="mock", event_id="evt-1", provider_reference=references["email"],
        status=MessageStatus.DELIVERED, payload={"status": "delivered"}, received_at=now,
    )

    knowledge = SQLiteKnowledgeBase(tmp_path / "knowledge.sqlite")
    content_hash = knowledge.ingest(
        document_id="policy-1", source="operations-manual", version="3",
        effective_at=datetime(2026, 7, 1, tzinfo=UTC), ingested_at=now,
    ...
    suggestions = suggest_capabilities(
        CapabilityState(
            has_stock_source=False, has_pricing_rule=True, has_delivery_provider=False,
            has_customer_consent=True, has_message_channel=True,
        )
    )
