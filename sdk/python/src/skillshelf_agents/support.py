...
def star_repository(*, confirmed: bool) -> str:
    if not confirmed:
        raise ValueError("Starring changes the authenticated GitHub account; pass --yes after explicit consent.")
    gh = shutil.which("gh")
    if gh is None:
        raise RuntimeError("GitHub CLI is not installed; open the repository and star it manually.")
    _run([gh, "auth", "status"])
    check = subprocess.run([gh, "api", "--method", "GET", f"/user/starred/{REPOSITORY}"],
                           capture_output=True, text=True)
    if check.returncode == 0:
        return f"{REPOSITORY} is already starred."
    _run([gh, "api", "--method", "PUT", "-H", "Accept: application/vnd.github+json",
          f"/user/starred/{REPOSITORY}"])
    verify = subprocess.run([gh, "api", "--method", "GET", f"/user/starred/{REPOSITORY}"],
                            capture_output=True, text=True)
