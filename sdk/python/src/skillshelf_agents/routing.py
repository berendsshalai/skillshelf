...
def route_task(task: str, explicit_agent: str | None = None) -> RouteDecision:
    if explicit_agent:
        return RouteDecision(direct_agent=explicit_agent, candidate_agents=[explicit_agent], confidence=1.0,
                             reasons=["explicit agent selection"], requires_master=False)
    ...
    return RouteDecision(direct_agent=agent if confidence >= 0.90 else None, candidate_agents=candidates,
                         confidence=confidence, reasons=[f"matched {best} routing signal(s)"],
